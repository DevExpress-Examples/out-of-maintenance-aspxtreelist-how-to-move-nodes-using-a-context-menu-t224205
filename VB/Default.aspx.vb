...
'INSTANT VB NOTE: The variable context was renamed since Visual Basic does not handle local variables named the same as class members well:
		Dim context_Conflict = New NorthwindDataClassesDataContext()
		Dim currentNode = context_Conflict.Employees.Where(Function(empl) empl.EmployeeID = Convert.ToInt32(param(0))).FirstOrDefault()
		Dim copyNode = context_Conflict.Employees.Where(Function(empl) empl.EmployeeID = Convert.ToInt32(param(1))).FirstOrDefault()
		If ProcessNodes(tree.FindNodeByKeyValue(copyNode.EmployeeID.ToString()), currentNode.EmployeeID.ToString()) Then
			...
			context_Conflict.SubmitChanges()
			tree.DataBind()
		Else
